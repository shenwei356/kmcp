...
`, os.Args[0]))
	...
	outfh.WriteString("alen\tacov\tpident\tqcov\tqkmers\tskmers\tmatches\tmismatches\tgaps\tquery\tsubject\tqlen\tslen\tqseq\tsseq\n")
	...
	for chunk := range reader.Ch {
		...
		for _, data = range chunk.Data {
			...
			outfh.WriteString(fmt.Sprintf("%d\t%f\t%f\t%f\t%d\t%d\t%d\t%d\t%d\t%s\t%s\t%d\t%d\t%s\t%s\n",
				m.ALen, m.AQov, m.Pident, m.KQcov, m.KQ, m.KS, m.KM, m.Mismatch, m.Gap,
...
func parseBlastResult(line string, numFields int, items *[]string) (BlastRecord, bool) {
	...
	m.Gap, err = strconv.Atoi((*items)[13])
	if err != nil {
		checkError(fmt.Errorf("fail to parse gap: %s", (*items)[13]))
	}
